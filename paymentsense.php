...
?><?php
class plgHikashoppaymentPaymentsense extends hikashopPaymentPlugin
{
	var $multiple = true;
	var $name = 'Paymentsense';
	...
	function onPaymentDisplay(&$order,&$methods,&$usable_methods)
	{
	    if(!empty($methods))
	    {
	        ...
	        {
	            if($method->payment_type!='paymentsense' || !$method->enabled)
	            {
	                continue;
	            }
	...
	function onBeforeOrderCreate(&$order,&$do) 
	{
		...
		if($order->order_payment_method != 'paymentsense') 
		{
			return true;
		}
		$db =& JFactory::getDBO();
		...
		{
			$app->enqueueMessage('The PaymentSense payment plugin needs the CURL library installed but it seems that it is not available on your server. Please contact your web hosting to set it up.','error');
			return false;
		}
		...
		do 
		{
			...
			if( $err == 0 ) 
			{
				...
				if( preg_match('#<StatusCode>([0-9]+)</StatusCode>#iU', $ret, $soapStatus) ) 
				{
					...
					if( preg_match('#<Message>(.+)</Message>#iU', $ret, $Transactionmessage) )
					{
					...
					if( $status != 50 ) 
					{
						...
						switch( $status ) 
						{
							case 0:
								...
								$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION','PaymentSense','Accepted'));
								$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','PaymentSense','Accepted')).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->order_status)."\r\n\r\n".$order_text;
								$mailer->setBody($body);
								$mailer->Send();
							...
							case 5:
								// Declined						       
							    $app->enqueueMessage('Transaction declined: ' . $Transactionmessage);							
								$do = false;
								break;
							...
							default:
								...
								$app->enqueueMessage('PaymentSense Error ('.$status.') :' . $msg);
								$do = false;
								break;
						}
					}
			...
			if( !$soapSuccess ) 
			{
				...
				if( $cpt >= 2 ) 
				{
					...
					if( $gwId > 3 ) 
					{
						$app->enqueueMessage('Impossible to contact the PaymentSense payment gateway.');
						$do = false;
						$soapSuccess = true;
						break;
					}
    ...
    function onAfterOrderConfirm(&$order,&$methods,$method_id) 
    {
        global $Itemid;
        $pluginsClass = hikashop::get('class.plugins');
        $elements = $pluginsClass->getMethods('payment','paymentsense');
        if(!empty($Itemid))
        {

            $element->payment_params->url_itemid = $Itemid;
        }
		...
		// Hosted
		//
		if( $method->payment_params->api == 'hosted') 
		{
			...
			$notify_url_p = 'option=com_hikashop&ctrl=checkout&task=notify&notif_payment=paymentsense&tmpl=component&lang='.$locale;
			$return_url_p = 'option=com_hikashop&ctrl=checkout&task=notify&notif_payment=paymentsense&tmpl=component&paymentsense_return=1&lang='.$locale;
			...
			if( $method->payment_params->api == 'hosted' ) 
			{
				...
				$vars = Array(
					...
					"TransactionType" => $method->payment_params->instant_capture,
					"TransactionDateTime" => date('Y-m-d H:i:s O'),
					"CallbackURL" => $server_url . '?' . $return_url_p,
					"OrderDescription" => 'Order ID '. $order->order_number . ', ItemID: ' . $element->payment_params->url_itemid,
				...
				$vars['HashDigest'] = $this->paymentSenseHash($method->payment_params->hash_method, $vars, $method->payment_params->sharedkey);
		...
		else 
		{
			...
			if( !empty($ref) ) 
			{
				...
				$vars = array(
					...
					'ret' => HIKASHOP_LIVE . 'index.php?option=com_hikashop&ctrl=checkout&task=threedsecure&3dsecure_payment=paymentsense&orderid='.$order->order_id
				);
    ...
    function onPaymentNotification(&$statuses)
	{
		...
		$vars = array(
		    ...
		    'AddressNumericCheckResult' => @$_POST["AddressNumericCheckResult"],
		    'PostCodeCheckResult' => @$_POST["PostCodeCheckResult"],
		    'CV2CheckResult' => @$_POST["CV2CheckResult"],
		    'ThreeDSecureAuthenticationCheckResult' => @$_POST["ThreeDSecureAuthenticationCheckResult"],
			'Amount' => @$_POST['Amount'],
		...
		$vars['Calculated_HashDigest'] = $this->paymentSenseHash($element->payment_params->hash_method, $vars, $element->payment_params->sharedkey);
		...
		if( $vars['MerchantID'] != $element->payment_params->merchantid ) 
		{
			$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','PaymentSense').'invalid merchant id');
			$body = JText::sprintf("Hello,\r\n A PaymentSense notification was refused because the response from the merchant identifier was invalid")."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
		...
		{
			$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','PaymentSense').'invalid currency');
			$body = JText::sprintf("Hello,\r\n A PaymentSense notification was refused because the response from the currency was invalid")."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
			$msg = ob_get_clean();
			echo 'StatusCode=30&Message='.$msg;
		...
		if( $_POST['HashDigest'] != $vars['Calculated_HashDigest'] ) 
		{
			...
			$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Payments').'invalid hash');
			$body = JText::sprintf("Hello,\r\n A PaymentSense notification was refused because the response from the hash was invalid")."\r\n\r\n".$order_text;
			$mailer->setBody($body);
			$mailer->Send();
			$msg = ob_get_clean();
			echo 'StatusCode=30&Message=' . $msg;
		...
		//Redirect the customer depending of order status
		
		$app =& JFactory::getApplication();
		
		switch($vars['StatusCode'])
		{
		    ...
		    case 0:
		        ...
		        $order->history->history_payment_id = $method->payment_id;
		        $order->history->history_payment_method =$method->payment_type;
		        $order->history->history_data = 'UUID: ' . $vars['OrderID'] . "\n" . 'CrossReference: ' . $vars['CrossReference'] . "\n" . ob_get_clean();
		        $order->history->history_type = 'payment';
		    ...
		    case 5:
		          ...
		          if($vars['Message'] != '')
		          {
		              $app->enqueueMessage('Transaction declined: ' . $vars['Message']);
		          }
		          else 
		          {
		              $app->enqueueMessage('Transaction declined');
		    ...
		    case 20:
		        ...
		        $previouosTransactionMessage = $vars['PreviousStatusCode'];
		        break;
		    case 30:
		         $app->enqueueMessage('Transaction declined.');
    ...
    function onThreeDSecure(&$statues) 
    {
		...
		$return_url = $httpsHikashop.'index.php?option=com_hikashop&ctrl=checkout&task=after_end&order_id='.$order->order_id;
		//$cancel_url = $httpsHikashop.'index.php?option=com_hikashop&ctrl=order&task=cancel_order';
		$cancel_url = HIKASHOP_LIVE.'index.php?option=com_hikashop&ctrl=order&task=show&order_id='.$order->order_id;
					...
					if( $status != 50 ) 
					{
						...
						switch( $status ) 
						{
							...
							case 5:
								...
								$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION','PaymentSense','Transaction declined'));

								$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','PaymentSense','Declined')).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->order_status)."\r\n\r\n".$order_text;
								$mailer->setBody($body);
								$mailer->Send();
								$orderClass->save($order);
							...
							default:
								...
								$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION','PaymentSense','Transaction error ('.$status.')'));

								$order_text = 'PaymentSense Error ('.$status.') : ' . $msg . "\r\n\r\n" . $order_text;
								$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','PaymentSense','Error')).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->order_status)."\r\n\r\n".$order_text;
								$mailer->setBody($body);
								$mailer->Send();
								$orderClass->save($order);
	...
	{
		    $element->payment_name='Paymentsense';
			$element->payment_description='You can pay by credit card using this payment method';
			$element->payment_images='VISA,Maestro,MasterCard,JCB';
			$element->payment_type=$this->paymentsense;
			$element->payment_params=null;
			$element->payment_params->hosted_mode='POST';
			...
			$element->payment_params->gw_entrypoint='paymentsensegateway.com';
    ...
    function paymentSenseHash($hash_method, &$data, $key) 
	{
		$str = '';
		foreach($data as $k=>$v) 
		{ 
